...
@section('content')
                    ...
                    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
                        ...
                        <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
                            <a href="{{ route('customer.consultation.create') }} " class="btn btn-primary shadow-md mr-2">Ajukan Pembuatan Produk</a>
                        </div>
                                    ...
                                    <div class="chat__chat-list overflow-y-auto scrollbar-hidden pr-1 pt-1 mt-4">
                                        @php
                                            $groups = $consultations->groupBy(function($c) {
                                                return optional($c->teacher)->id ?? $c->assigned_teacher_id;
                                            });
                                        @endphp

                                        @forelse($groups as $teacherId => $group)
                                            @php
                                                $firstConsult = $group->first();
                                                $teacher = $firstConsult->teacher;
                                            ...
                                            <div class="intro-x cursor-pointer box relative flex items-center p-5 mt-5" data-teacher-id="{{ $teacherId }}">
                                                <div class="w-12 h-12 flex-none image-fit mr-1">
                                                    <img alt="{{ optional($teacher)->name }}" class="rounded-full" src="{{ optional($teacher->profile)->avatar ?? asset('dist/images/profile-12.jpg') }}">
                                                    <div class="w-3 h-3 bg-success absolute right-0 bottom-0 rounded-full border-2 border-white dark:border-darkmode-600"></div>
                                                </div>
                                                <div class="ml-2 overflow-hidden">
                                                    <div class="flex items-center">
                                                        <a href="{{ route('customer.konsultation.chat', ['id' => $openConsultation->id]) }}" class="font-medium load-chat">{{ optional($teacher)->name ?? 'Unknown' }}</a>
                                        ...
                                        @empty
                                            <div class="text-slate-500 p-4">You have no chats yet. Start a chat with a teacher.</div>
                                        @endforelse
                                    </div>
                                </div>
                                <div id="friends" class="tab-pane" role="tabpanel" aria-labelledby="friends-tab">
                                    ...
                                    <div class="chat__user-list overflow-y-auto scrollbar-hidden pr-1 pt-1">
                                        @foreach($teachers ?? [] as $t)
                                            <div
                                                        onclick="loadChat('{{ route('customer.konsultation.start', ['teacher' => $t->id]) }}')"
                                                        class="cursor-pointer box relative flex items-center p-5 mt-5 hover:bg-slate-100 transition"
                                                    >
                                                <div class="w-12 h-12 flex-none image-fit mr-1">
                                                    <img alt="{{ $t->name }}" class="rounded-full" src="{{ optional($t->profile)->avatar ?? asset('dist/images/profile-3.jpg') }}">
                                                    <div class="w-3 h-3 bg-success absolute right-0 bottom-0 rounded-full border-2 border-white dark:border-darkmode-600"></div>
                                                </div>
                                                <div class="ml-2 overflow-hidden">
                                                    <div class="flex items-center"> <a href="#" class="font-medium">{{ $t->name }}</a> </div>
                                                    <div class="w-full truncate text-slate-500 mt-0.5">{{ $t->email }}</div>
                                                </div>
...
// Hapus unread badge ketika buka chat
...
// Refresh chat list every 7 detik
function refreshChatList() {
    fetch('{{ route('customer.konsultation') }}', { headers: {'X-Requested-With': 'XMLHttpRequest'} })
        .then(r => r.text())
        .then(html => {
            try {
                const parser = new DOMParser();
                ...
                // Update per teacherId
                newList.querySelectorAll('.intro-x.cursor-pointer.box[data-teacher-id]').forEach(newCard => {
                    const teacherId = newCard.getAttribute('data-teacher-id');
                    const oldCard = oldList.querySelector(`.intro-x.cursor-pointer.box[data-teacher-id="${teacherId}"]`);
                    if (!oldCard) return;
...
document.addEventListener('submit', function (e) {
    ...
    .then(() => {
        loadChat(`/customer/konsultation/${consultationId}/chat`);
        form.reset();
    })
